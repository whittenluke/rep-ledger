...
export interface TemplateExerciseRow {
  ...
  target_weight: number | null
  notes: string | null
  exercises: { name: string; primary_muscle: string | null; type: 'reps' | 'time'; image_url: string | null } | null
...
export function useTemplateExercises(templateId: string | null) {
  ...
  const fetch = useCallback(async () => {
    ...
    const { data: teData, error: e } = await supabase
      ...
      .select('id, template_id, exercise_id, position, target_sets, target_reps, target_duration_seconds, target_weight, notes, exercises(name, primary_muscle, type, image_url)')
